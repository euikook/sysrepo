        ...
        int keep_cur_child)
{
    ...
    if ((!cur_pid && !cur_conn_ptr) || (cur_pid != getpid()) || (cur_conn_ptr != conn_ptr)) {
        if (cur_meta_own) {
            ...
            sr_edit_del_meta_attr(diff_node, "pid");
            sr_edit_del_meta_attr(diff_node, "conn-ptr");
        }

        /* add attrs of the new connection */
        sprintf(pid_str, "%ld", (long)getpid());
        if (lyd_new_meta(LYD_CTX(diff_node), diff_node, NULL, SR_YANG_MOD ":pid", pid_str, 0, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
            return err_info;
        }
        sprintf(conn_str, "%" PRIuPTR, (uintptr_t)conn_ptr);
        if (lyd_new_meta(LYD_CTX(diff_node), diff_node, NULL, SR_YANG_MOD ":conn-ptr", conn_str, 0, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
            return err_info;
        }
        ...
        LY_LIST_FOR(lyd_child_no_keys(diff_node), child) {
            ...
            if (!meta_own) {
                if (lyd_new_meta(LYD_CTX(diff_node), child, NULL, SR_YANG_MOD ":pid", pid_str, 0, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
                    return err_info;
                }
                if (lyd_new_meta(LYD_CTX(diff_node), child, NULL, SR_YANG_MOD ":conn-ptr", conn_str, 0, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_CTX(diff_node));
                    return err_info;
                }
            }
...
sr_edit_find_match(const struct lyd_node *first_node, const struct lyd_node *edit_node, struct lyd_node **match_p)
{
    ...
    if (!edit_node->schema) {
        ...
        opaq = (struct lyd_node_opaq *)edit_node;
        switch (opaq->format) {
        case LY_PREF_XML:
            mod = ly_ctx_get_module_implemented_ns(LYD_CTX(edit_node), opaq->name.module_ns);
            break;
        case LY_PREF_JSON:
            mod = ly_ctx_get_module_implemented(LYD_CTX(edit_node), opaq->name.module_name);
            break;
        default:
            SR_ERRINFO_INT(&err_info);
            return err_info;
        ...
        if (mod) {
            ...
            schema = lys_find_child(edit_node->parent ? edit_node->parent->schema : NULL, mod, opaq->name.name, 0, 0, 0);
        }
        if (schema) {
